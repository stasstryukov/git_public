...
commands = {
    "192.168.1.103": ["sh ip int br", "sh ip route | ex -"],
    "192.168.1.101": ["sh ip int br", "sh int desc"],
    "192.168.1.102": ["sh int desc"],
}
...
def send_command(ip,commands):
	...
	with ConnectHandler(**ip) as ssh:
		...
		for command in commands:
			...
			output = output + prompt + command + '\n' + result + '\n'
		return output

def send_commands(devices,commands_dict,filename, limit=3):
...
if __name__ == "__main__":
	...
	send_commands(devices,commands,"result.txt")
